...
# --- ROUTING DI BASE ---
# Abilita l'IP forwarding nel kernel
sysctl -w net.ipv4.ip_forward=1

# Imposta la rotta predefinita: tutto il traffico destinato all'esterno della LAN2 viene inviato all'eFW
ip route add default via 10.20.1.1

# --- INIZIO REGOLE FIREWALL (IPTABLES) ---
# Elimina tutte le regole preesistenti nella catena FORWARD per partire da zero
iptables -F FORWARD

# Imposta la policy di default blocca automaticamente tutto il traffico in transito che non corrisponde a regole specifiche
iptables -P FORWARD DROP

# REGOLA IN USCITA: Consente ai dispositivi della LAN2 collegati su eth1 di comunicare verso l'esterno uscendo da eth0
